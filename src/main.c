...
static void echo_task(void *arg)
{
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            ESP_LOGI("Recv str", "%s", data);
        }

        vTaskDelay(10 / portTICK_PERIOD_MS); // Add a small delay to avoid task starvation
    }
}

void app_main(void)
{
    xTaskCreate(echo_task, "USB_SERIAL_JTAG_echo_task", ECHO_TASK_STACK_SIZE, NULL, 10, NULL);
}
